...
import com.yc.snack.user.dto.SessionKeyConstant;
...
public class MemberInfoController {
	...
	public ResultVO login(MemberInfo mf,HttpSession session) {
		...
		session.setAttribute(SessionKeyConstant.CURRENTMEMBERACCOUNT, memberInfo);
	...
	public ResultVO check(HttpSession session) {
		Object obj = session.getAttribute(SessionKeyConstant.CURRENTMEMBERACCOUNT);
		if(obj == null) {
	...
	public ResultVO sendCode(String receive,String nickName,HttpSession session) {
		...
		if(sendMailUtil.sendHtmlMail(receive, nickName, code)) {
			...
			timer.schedule(task, 180000);//3分钟后执行这个任务器
